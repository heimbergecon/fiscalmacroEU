...
foreach var in `vars' { 
	...
	* preallocate 
	...
	tw (rarea up90b`var' lo90b`var' h, fcolor("$mdred%15") lcolor(mdred) lw(none) lpattern(solid)) ///
		...
		graph export "$FIGUREDIR\pirf_`var'.jpg", fontface($grfont) replace
...
* Set up storage for Kleinbergen test
local nvar : word count `vars'
matrix KPF = J(`horizon'+1, `nvar', .)
local rownames ""
forvalues hh = 0/`horizon' {
    local rownames "`rownames' h`hh'"
}
matrix rownames KPF = `rownames'
matrix colnames KPF = `vars'
...
foreach var in `vars' { 
	...
	* preallocate 
	...
	tw (rarea up90biv`var' lo90biv`var' h, fcolor("$mblue%15") lcolor(mdred) lw(none) lpattern(solid)) ///
		...
		graph export "$FIGUREDIR\pirf_iv_`var'.jpg", fontface($grfont) replace

			local ivar = `ivar'+1

		}

matrix list KPF, format(%9.3f)
putexcel set "$TABLEDIR/KP_Fstats.xlsx", replace
putexcel A1 = matrix(KPF), names
...
cap g zero = 0
	...
	tw (rarea up90biv`var' lo90biv`var' h, fcolor("$mblue%15") lwidth(none) lpattern(solid)) ///
		...
		(line biv`var' h, lcolor("$mblue") lpattern(dash) lwidth(thick)) ///
		(rarea up90b`var' lo90b`var' h, fcolor("gs12%15") lwidth(none) lpattern(solid)) ///
		(rarea up68b`var' lo68b`var' h, fcolor("gs12%40") lwidth(none) lpattern(solid)) ///
		(line b`var' h, lcolor("gs12") lpattern(dash) lwidth(thick)) ///
		(line zero h, lc(black) clw(vvthin)) if h<=`horizon', ///
		title("`varname'", color(black) size(10)) xtitle("Years", size(7)) ///
		ytitle("`labname'", size(7))   ///
		xlabel(0(1)`horizon', labsize(7)) ///
		...
		graph export "$FIGUREDIR\pirf_joint_`var'.jpg", fontface($grfont) replace
...
graph export "$FIGUREDIRC\iv_vs_ols.jpg", replace


// IV results only:
graph use "$FIGUREDIR\pirf_iv_log_REALGDP.gph", name(g1, replace)
graph use "$FIGUREDIR\pirf_iv_unemp.gph", name(g2, replace)
graph use "$FIGUREDIR\pirf_iv_INFL.gph", name(g3, replace)
graph use "$FIGUREDIR\pirf_iv_gini_disp.gph", name(g4, replace)

grc1leg2 g1 g2 g3 g4, cols(4) ysize(3) xsize(12)  loff 

graph export "$FIGUREDIRC\baseline_iv.jpg", replace
*******************************************************************************

preserve
keep h bivlog_REALGDP up90bivlog_REALGDP lo90bivlog_REALGDP up68bivlog_REALGDP lo68bivlog_REALGDP bivunemp up90bivunemp  lo90bivunemp up68bivunemp lo68bivunemp bivINFL up90bivINFL lo90bivINFL up68bivINFL lo68bivINFL bivgini_disp up90bivgini_disp lo90bivgini_disp up68bivgini_disp lo68bivgini_disp
		
export excel using "$TABLEDIR\FiscalConsolidation_Headline.xlsx", replace firstrow(variables)
restore		
